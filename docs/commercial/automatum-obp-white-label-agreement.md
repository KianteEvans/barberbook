# White Label Delivery Agreement

**Managed Partner Development**

This Agreement is made as of [to supply: Effective Date] between
**[to supply: Automatum legal entity name]**, a [to supply: entity type] organized under the
laws of [to supply: state of organization], company number [to supply: registration number],
of [to supply: Automatum principal address] ("**Automatum**"), and
**[to supply: Only Best Practices legal entity name]**, a [to supply: entity type] organized
under the laws of [to supply: state of organization], company number [to supply: registration
number], of [to supply: OBP principal address] ("**OBP**"). Each is a "Party".

## Recitals

**A.** Automatum publishes software through AWS Marketplace, holds the seller of record
position for its listing, and issues private offers and Selling Authorizations under it. OBP
operates a managed channel function and licenses and operates a partner system of record.

**B.** The Parties have developed Managed Partner Development, under which a software vendor
with a live AWS Marketplace listing engages a single provider to build and run its reseller
channel on AWS.

**C.** Automatum contracts with, invoices and faces each client. OBP performs the channel
function as Automatum's subcontractor under Automatum's brand and is not presented to the
client as a separate provider. The arrangement is non-exclusive.

The Parties agree as follows.

---

## 1. Definitions and Interpretation

### 1.1 Definitions

"**Active Partner**" means a Recruited Partner that has completed at least one Transaction in
the preceding 365 days, whatever its onboarding status.

"**ACE**" means the AWS Partner Central APN Customer Engagements program. "**AWS**" means
Amazon Web Services, Inc. and its affiliates. "**AWS Marketplace**" means the AWS Marketplace
digital catalog. "**CPPO**" means a Channel Partner Private Offer made through AWS
Marketplace.

"**AWS Program Terms**" means the AWS Partner Network terms, the AWS Marketplace Seller Terms
and Conditions, the AWS Customer Agreement, the ACE program terms, and any other AWS terms,
policies or program rules applicable to a Party, as amended by AWS from time to time.

"**Automatum Brand**" means the Automatum names, logos, marks, domains, email domains,
templates and other brand assets Automatum makes available to OBP.

"**Business Day**" means a day other than a Saturday, Sunday or public holiday in the State of
New York. "**Business Hours**" means 9:00 a.m. to 6:00 p.m. Eastern Time on a Business Day,
unless an Engagement Order states a different coverage window.

"**Channel Review**" means a governance session with a Registered Client covering partner
pipeline, partner performance, the headline metric and remediation, at the Tier cadence in
Schedule 1.

"**Client**" means a software vendor that contracts with Automatum for the Solution.
"**Client Agreement**" means the agreement under which Automatum supplies the Solution to a
Client. "**Client Fees**" means the fees payable by a Registered Client under its Client
Agreement for the Solution, excluding Taxes and fees for services outside the Solution.

"**Client Data**" means all data relating to a Registered Client, its Recruited Partners and
its channel created, collected, processed or stored in delivering the Services, including
partner records, deal registrations, commission records, offer records and reporting outputs.

"**Client Registry**" means the register of Registered Clients maintained under clause 4.

"**Client Source**" means whether a Registered Client is OBP-Sourced, Automatum-Sourced, or
Joint or ACE-Sourced, determined under clause 4.7. "**Client Status**" means whether a
Registered Client is a Net New Client or an Existing Automatum Client, determined under clause
4.7.

"**Collected Client Fees**" means, for a Registered Client and an Engagement Month, the Client
Fees actually received by Automatum for that month, net of Taxes, payment processing fees, and
any AWS Marketplace listing fee to the extent clause 11.9 applies.

"**Deal Desk Request**" means a request by or for a Recruited Partner or Registered Client to
structure, price, register or progress a CPPO, submitted through the intake channel in the
form specified in the Engagement Order.

"**Engagement**" means supply of the Solution to one Registered Client under one Engagement
Order. "**Engagement Month**" means a calendar month, or part of one, during which an
Engagement is in effect. "**Engagement Year**" means each 12 consecutive months from the start
date in an Engagement Order.

"**Engagement Order**" means an order in the form of Schedule 6, executed by both Parties,
adding a Client to the Client Registry and engaging OBP for that Client.

"**Existing Automatum Client**" means a Client that, at the date its Engagement Order is
executed or in the 90 days before, subscribes or subscribed to Automatum's Enterprise tier or
any other paid Automatum subscription. "**Net New Client**" means any other Client.

"**Issuance-Ready Package**" means a completed CPPO structure, pricing, partner and end
customer details and supporting documentation, in the form stated in the Engagement Order,
delivered by OBP so that Automatum can issue the offer.

"**Losses**" means losses, liabilities, damages, judgments, settlements, fines, penalties,
costs and expenses, including reasonable attorneys' fees.

"**OBP Materials**" means the methodologies, playbooks, templates, partner agreement and
margin schedule forms, curricula, enablement content, processes and know-how OBP owns or
licenses and uses in delivering the Services, together with the PSR.

"**OBP Service Revenue**" means OBP's share of Service Revenue under clause 11.2.

"**Offer-Issuing Partner**" means, for a Registered Client and a measurement period, a
Recruited Partner that issued at least one CPPO against that Client's listing in that period,
as recorded in the PSR and determined under Schedule 4.

"**Partner System of Record**" or "**PSR**" means the partner management platform licensed and
operated by OBP under clause 10 and Schedule 5, providing partner portal, onboarding, deal
registration and commission tracking, with HubSpot and Salesforce synchronization.

"**Personal Data**" means information relating to an identified or identifiable natural person
subject to any Privacy Law. "**Privacy Law**" means any law on the protection of Personal Data
applicable to a Party's performance.

"**Platform Revenue**" means $9,999 per Engagement Year, being the price of Automatum's
Enterprise tier. It belongs to Automatum in every case.

"**Recruited Partner**" means a reseller, systems integrator, managed service provider or
other channel partner OBP recruits, contracts or onboards for a Registered Client under this
Agreement.

"**Registered Client**" means a Client entered in the Client Registry under clause 4 for which
an Engagement Order is or has been in effect.

"**Selling Authorization**" means an authorization issued by Automatum through AWS Marketplace
permitting a Recruited Partner to resell a listed product.

"**Service Credit**" means a credit Automatum must give a Registered Client under its Client
Agreement for failing to meet a service level.

"**Service Revenue**" means the Collected Client Fees for an Engagement Year less the Platform
Revenue. Clause 11.2 divides it.

"**Services**" means the services OBP performs under clause 5 and Schedule 1. "**Solution**"
means the Managed Partner Development solution in Schedule 1. "**Tier**" means Tier 1, 2 or 3
as described in Schedule 1.

"**Taxes**" means sales, use, value added, goods and services, excise and similar transaction
taxes, excluding taxes on a Party's net income.

"**Transaction**" means a completed purchase through AWS Marketplace sourced by a Recruited
Partner and attributable to a Registered Client's listing.

### 1.2 Interpretation

Headings do not affect interpretation. "Including" is not limiting. "$" means United States
dollars and all amounts exclude Taxes. References to clauses and Schedules are to this
Agreement, and the Schedules form part of it. The singular includes the plural. A reference to
a statute or AWS Program Term is to it as amended or replaced. No rule of construction
operates against the drafter.

---

## 2. Structure and Order of Precedence

**2.1** This Agreement consists of these clauses, the Schedules, and each Engagement Order.
An Engagement Order incorporates this Agreement by reference and takes effect when signed by
both Parties.

**2.2** On conflict, the following prevails in descending order: an Engagement Order, but only
for its own Engagement and only where it expressly identifies the provision it varies; these
clauses; the Schedules.

**2.3** A Client Agreement is not part of this Agreement, does not bind OBP, and cannot vary
it. Clause 6.5 governs flow-down.

---

## 3. Appointment

**3.1 Appointment.** Automatum appoints OBP as a delivery subcontractor to perform the
Services for Registered Clients under the Automatum Brand, and OBP accepts.

**3.2 Non-exclusive.** The appointment is non-exclusive both ways. Automatum may perform the
Services itself or appoint others for any client, and OBP may provide the same or similar
services to anyone, including another AWS Marketplace seller. This is subject only to clause
15, which operates solely in respect of Registered Clients and Recruited Partners.

**3.3 No volume commitment.** Automatum commits to no minimum number of Engagements,
Registered Clients or Client Fees, and OBP has not relied on any forecast, including any
projection in the feasibility study for the Solution.

**3.4 No agency.** OBP is an independent contractor, not Automatum's agent, employee, partner
or joint venturer, and may not bind Automatum except as Automatum authorizes in writing for a
specific matter.

**3.5 Prime responsibility.** Automatum is the prime contractor and is responsible to each
Registered Client for the Solution as a whole. As between the Parties, OBP is responsible for
the Services and for its personnel and subcontractors.

---

## 4. Client Registry and Engagement Orders

**4.1 Purpose.** The Client Registry is the sole record of which Clients are subject to this
Agreement. A Client is within scope only once entered in it.

**4.2 Adding a Client.** A Client is added when the Parties execute an Engagement Order.
Automatum maintains the Registry and provides a current copy with each monthly statement under
clause 11.5 and on request.

**4.3 Engagement Order contents.** Each Engagement Order states at least: the Client's legal
name; the Tier; the Client Status and Client Source; the Engagement start and Engagement Year
dates; the Client Fees and the resulting OBP Service Revenue; the personnel named under clause
9; the PSR seat allocation; and any variation permitted by clause 2.2.

**4.4 Prior relationships.** Before an Engagement Order is executed, OBP may notify Automatum
in writing, with reasonable evidence, of a pre-existing relationship with the proposed Client
or a named partner of it. A relationship so notified is a "Prior Relationship" and is carved
out of clause 15.2. A relationship not notified before execution is not a Prior Relationship.

**4.5 Refusal.** OBP may decline a proposed Engagement Order within 5 Business Days, acting
reasonably, including for lack of capacity at the proposed Tier, because the proposed Client
competes with an existing OBP client, or because the proposed Client's existing Client
Agreement does not meet clause 6.5. Declining is not a breach, and Automatum may then serve
that Client itself or through another provider.

**4.6 Removal.** A Client remains in the Registry while its Engagement is in effect and for
the period in clause 15.2, then is marked closed. Closure does not affect clause 23.6.

**4.7 Client Status and Client Source.** Each Engagement Order records two classifications,
fixed at execution and not revisited during that Engagement Year:

  a. **Client Status** - Net New Client or Existing Automatum Client. Automatum will disclose
     before execution whether the proposed Client holds or held any paid Automatum
     subscription in the preceding 90 days, with reasonable evidence on request. Client Status
     determines the division of Service Revenue under clause 11.2.
  b. **Client Source** - OBP-Sourced, Automatum-Sourced, or Joint or ACE-Sourced, by which
     Party first made documented contact with the Client about the Solution, and Joint or
     ACE-Sourced where the opportunity came through ACE or neither Party can show first
     contact.

  Client Status is re-determined on a renewal Engagement Order. A dispute about either
  classification is escalated under clause 26.2.

---

## 5. OBP Delivery Obligations

**5.1 The Services.** For each Engagement OBP will, at the applicable Tier and to the service
levels in Schedule 4, using suitably skilled personnel and with reasonable skill and care:

  a. identify, qualify, recruit and onboard Recruited Partners against the ideal partner
     profile in the Engagement Order, up to the number stated for that Tier, screening each
     candidate before onboarding against sanctions and denied party lists, adverse media,
     beneficial ownership including whether any owner or controller is a government official,
     AWS Partner Network standing, and financial standing;
  b. draft, issue and administer the partner agreement and margin schedule;
  c. guide each partner through AWS registration, tax and banking details and service-linked
     role creation;
  d. deliver enablement and training sessions at the Tier cadence, covering the Client's
     product, offer mechanics, deal registration and the margin schedule;
  e. operate the CPPO deal desk, delivering an Issuance-Ready Package to Automatum within the
     OBP leg in Schedule 4;
  f. prepare and submit to Automatum requests for recurring Selling Authorizations, and track
     their status in the PSR;
  g. deliver Channel Reviews at the Tier cadence;
  h. stand up, configure, host, operate, support and maintain the PSR under clause 10 and
     Schedule 5, within the OBP Service Revenue and not separately chargeable;
  i. support configuration of AWS Partner Revenue Measurement attribution so far as within
     OBP's control, and where action is required of Automatum, the Client or AWS, specify what
     is required and verify the result;
  j. file partner-sourced opportunities to ACE under the APN account in Schedule 8 and no
     other, tracked distinctly from any baseline quota; and
  k. deliver the monthly reporting pack in Schedule 4 within 5 Business Days of month end, in
     the Automatum Brand and in a form Automatum can pass to the Client unaltered.

**5.2 Recruitment is a ceiling, not a minimum.** The partner numbers in Schedule 1 are the
maximum OBP must recruit and onboard. OBP does not warrant, and need not procure, that any
minimum number of partners is recruited, onboarded, authorized, transacts or becomes an Active
Partner, and Automatum will not represent otherwise. This clause prevails over any contrary
reading of Schedule 1.

**5.3 Partner contracting.** Each partner agreement is executed between the Registered Client,
or Automatum as its agent, and the Recruited Partner. **OBP is not a party to any partner
agreement as principal** and will not enter into one in its own name. OBP administers them as
agent under the authority in the Engagement Order and no wider.

**5.4 Deal desk boundary.** OBP's deal desk obligation ends on delivery of the Issuance-Ready
Package. Issuance is Automatum's under clause 6.3.

**5.5 First line of support.** OBP is first line for the channel, the deal desk and the PSR,
through the personnel named under clause 9, routed through the single Automatum-branded intake
in Schedule 3.

**5.6 Instructions.** OBP will follow Automatum's reasonable written instructions relating to
the Services, except where doing so would breach this Agreement, an AWS Program Term or any
law, and will notify Automatum promptly if it believes an instruction would.

**5.7 Subcontracting.** OBP may subcontract only to subcontractors Automatum approves in
writing, remains fully liable for them, and may not subcontract any role named under clause 9.

---

## 6. Automatum Retained Obligations

**6.1 Listing and seller of record.** Automatum will maintain its AWS Marketplace listing, its
seller of record registration, and the AWS Partner Network status and entitlements needed to
issue private offers, CPPOs and Selling Authorizations.

**6.2 AWS relationship.** Automatum owns the AWS relationship for the listing and the
Solution, including obtaining the confirmation in clause 14.2.

**6.3 Issuance.** Automatum will issue private offers, CPPOs and Selling Authorizations
promptly on receiving an Issuance-Ready Package or Selling Authorization request, and within
the Automatum leg in Schedule 4.

**6.4 Client contracting and collection.** Automatum will contract with each Client, invoice
the Client Fees and pursue collection under clause 11.

**6.5 No-worse-than covenant.** Automatum warrants that no Client Agreement for a Registered
Client contains, in respect of services OBP performs, a service level stricter than Schedule
4, a service credit percentage or at-risk amount above the caps in Schedule 4, a guaranteed
partner number, transaction volume, revenue outcome or activation rate, or a metric definition
inconsistent with Schedule 4. Anything agreed beyond this baseline is Automatum's own risk, is
excluded from clause 8.4, and does not increase OBP's obligations. This covenant is the
consideration for the full flow-down in clause 8.4.

**6.6 No onward commitments.** Automatum will make no representation, warranty or commitment
to a Client or prospective Client about the Services, the Solution, partner numbers, offer
issuance or any metric beyond Schedules 1 and 4, and indemnifies OBP under clause 20.2 for
claims arising from any excess.

**6.7 Relief Events.** Each of the following is a "Relief Event": Automatum's failure to
perform clause 6.1, 6.2 or 6.3; its loss of the listing, seller of record position or AWS
Partner Network entitlements; a Registered Client's failure to perform an obligation on which
OBP depends; and any matter in clause 14.3. While a Relief Event continues, OBP is excused
from the affected obligation, the affected service levels do not run, and OBP Service Revenue
continues to accrue. OBP will notify Automatum promptly and use reasonable efforts to
mitigate.

---

## 7. White Label and Brand

**7.1 Brand licence.** Automatum grants OBP a non-exclusive, non-transferable, royalty-free
licence for the Term to use the Automatum Brand solely to perform the Services under Schedule
3. OBP acquires no other rights and all goodwill accrues to Automatum.

**7.2 Delivery under the Automatum Brand.** OBP will perform all client-facing and
partner-facing elements of the Services under the Automatum Brand, using Automatum-supplied
email addresses and domains, Automatum-branded documents, portals and correspondence, and the
protocol in Schedule 3.

**7.3 No OBP branding.** OBP will not use its own name, logos, marks, domains, email
addresses, templates or contact details in any client-facing or partner-facing material, and
will not identify itself to a Client or Recruited Partner as the provider of the Services,
except under clause 7.5.

**7.4 No direct client contact.** OBP will not contact a Registered Client outside the
channels and personas in Schedule 3, and will not contact a Registered Client at all about a
dispute between the Parties, including any payment dispute.

**7.5 Permitted disclosure.** OBP may disclose its role where required by law, a regulator, an
AWS Program Term, or a Client Agreement Automatum has notified to OBP in writing, giving
Automatum reasonable advance notice where it lawfully can.

**7.6 Employment status.** OBP is the sole employer of its personnel. Automatum does not
direct, supervise or control how they perform the Services. OBP's personnel will not state or
imply that they are employed by Automatum, and Schedule 3 states what they may and may not say.
Each Party will implement the measures in Schedule 3 against a finding of joint or
co-employment.

**7.7 No publicity.** Neither Party will refer to the other, to this Agreement, to any
Registered Client or to any Recruited Partner in any marketing material, case study, release,
website, pitch or public statement without the other's prior written consent.

**7.8 Automatum's own conduct.** Automatum will not disclose OBP's role to a Client or
Recruited Partner except as required by law, a regulator, an AWS Program Term or a Client
Agreement.

---

## 8. Service Levels and Service Credits

**8.1 Service levels.** OBP will meet the service levels in Schedule 4, which state for each
what starts and stops the clock, the measurement source, and the division between the OBP leg
and the Automatum leg.

**8.2 Split legs.** Because Automatum issues offers and Selling Authorizations as seller of
record, the deal desk and Selling Authorization service levels are measured in two legs. OBP
is responsible only for the OBP leg, and a failure of the Automatum leg gives rise to no
flow-down under clause 8.4. Where root cause analysis is genuinely inconclusive between the
legs, responsibility is apportioned equally and either Party may escalate under clause 26.2.

**8.3 Excused events.** A service level does not run, and no failure occurs, to the extent
delay is caused by a Relief Event, a Force Majeure Event, an incomplete or non-conforming Deal
Desk Request, a Recruited Partner's failure to complete AWS registration, tax, banking or
service-linked role requirements, a Registered Client's act or omission, or an act or omission
of AWS. A change in OBP's personnel is **not** an excused event and service levels continue to
run through any replacement under clause 9.

**8.4 Flow-down of Service Credits.** Where Automatum must give a Registered Client a Service
Credit, it may deduct that amount from the next monthly OBP Service Revenue for that Client,
but only where the failure was of the OBP leg and attributable to OBP; Automatum actually gave
the credit and did not give it ex gratia, as a concession, or in settlement of an unrelated
matter without OBP's prior written consent; Automatum applied every exclusion, cap and excused
event available to it; and Automatum notified OBP of the claim with supporting evidence within
10 Business Days of receiving it and gave OBP a reasonable opportunity to comment.

**8.5 Caps on flow-down.** Deductions under clause 8.4 are capped, per Registered Client, at
the lower of the credit actually given and 10% of that Client's gross monthly Client Fees per
Engagement Month, rising to 20% in a month in which two or more service levels are missed; and
at 15% of that Client's OBP Service Revenue per Engagement Year.

**8.6 Sole financial remedy.** Subject to clause 8.7, clause 8.4 is Automatum's sole financial
remedy for a failure to meet a service level.

**8.7 Chronic Failure.** "Chronic Failure" means 3 failures of the same service level in any
rolling 3 Engagement Months, or 6 failures across any service levels in an Engagement Year,
for the same Registered Client and attributable to OBP. Automatum may then require a root
cause analysis within 5 Business Days, a written remediation plan within 10 Business Days, and
replacement of the personnel named under clause 9 for that Engagement; and if the Chronic
Failure is not remedied within 30 days of the remediation plan, may terminate the affected
Engagement Order under clause 23.4 with transition assistance at OBP's cost.

---

## 9. Key Personnel

**9.1 Named roles.** Each Engagement Order names a Channel Manager, a Backup Channel Manager
and, at Tier 3, a Deal Desk Owner with a stated coverage window, each with the committed hours
and coverage window in Schedule 7.

**9.2 Approval.** Automatum approves initial appointees, not to be unreasonably withheld,
conditioned or delayed, and deemed given if Automatum does not object within 5 Business Days
of receiving the appointee's details.

**9.3 Minimum tenure.** OBP will not voluntarily remove or reassign a named person for 12
months or the remainder of the Engagement, whichever is shorter, unless the person resigns,
dies, becomes incapacitated, takes statutory leave, is dismissed for cause, or is removed at
Automatum's request.

**9.4 Replacement.** OBP will notify Automatum within 2 Business Days of becoming aware that a
named person will cease in the role. The Backup Channel Manager assumes it immediately. OBP
will propose a replacement of equivalent or greater experience within 10 Business Days and
provide a 10 Business Day handover overlap at its own cost. Clause 8.3 applies throughout.

**9.5 Removal.** Automatum may require removal of a named person from an Engagement for
material underperformance, misconduct, breach of Schedule 3, or repeated attributable service
level failures, and replacement is at OBP's cost. This is a request to remove the person from
the Engagement, not a direction to dismiss or discipline, and OBP remains solely responsible
for all employment decisions.

**9.6 No dedication.** Named personnel are assigned on a shared service basis and are not
exclusively dedicated unless the Engagement Order says so.

**9.7 Capacity ratios.** OBP will not exceed the accounts-per-owner ratios in Schedule 7.
Exceeding a ratio is a breach whether or not a service level has been missed, and Automatum
may require a remediation plan under clause 8.7 on that basis alone.

**9.8 Succession.** OBP will maintain a documented succession plan for each named role,
cross-train the Backup Channel Manager, and ensure client and partner knowledge is recorded in
the PSR rather than held only by individuals.

---

## 10. Partner System of Record

**10.1 Rights warranty.** OBP warrants that it holds all rights needed to provide and operate
the PSR as contemplated here, including to present it under the Automatum Brand and to grant
the access and licences in this clause, and that its own licence from the PSR vendor runs
beyond the expiry of the longest Engagement Order then in effect plus the transition period in
clause 24.1.

**10.2 White label configuration.** Each Registered Client's tenant is configured under the
Automatum Brand, with no PSR vendor or OBP branding visible to the Client or any Recruited
Partner, except where the vendor's licence makes removal impossible, in which case OBP will
notify Automatum before the tenant goes live.

**10.3 Data ownership and permitted use.** Client Data is owned by the Registered Client it
relates to and licensed to Automatum under that Client Agreement. OBP may process it only to
deliver the Services for that Client, and will not use it, identifiable or aggregated, for
benchmarking, product development, its own channel business, or any other OBP client.

**10.4 Standing data replica.** OBP will export Client Data to storage Automatum controls, in
the documented schema in Schedule 5, daily for transactional records and monthly as a full
snapshot. The Parties will test a restore quarterly and reconcile record counts.

**10.5 Audit record integrity.** OBP will maintain an append-only event log of the events
constituting each metric in Schedule 4, retain it for the Term plus 7 years, and make it
available to Automatum, a Registered Client and their auditors on reasonable notice. This
obligation survives termination.

**10.6 Administrative access.** Automatum holds administrative access to each Registered
Client's tenant throughout the Engagement.

**10.7 Vendor continuity.** OBP will procure and maintain from the PSR vendor a written
undertaking in the form of Schedule 5 Part D, under which on OBP's insolvency, cessation of
business, or termination of this Agreement for OBP's breach, the vendor will contract directly
with Automatum or its nominee for the remainder of the affected Engagement Orders plus the
transition period, at the same or benchmarked rates, with tenant data intact. This is a
condition to OBP's first Engagement Order. If the vendor will not give it, OBP will notify
Automatum before the first Engagement Order is executed, and Automatum may proceed, decline,
or require an alternative PSR.

**10.8 Step-in.** Automatum may step in on a Chronic Failure, an insolvency event affecting
OBP, a material service failure threatening a Registered Client's service levels, or an AWS
compliance event. OBP will then grant Automatum and any nominee access, credentials and
administrative rights to the PSR and a royalty-free licence to the OBP Materials for the
remainder of the affected Engagement Orders plus the transition period. Step-in does not
relieve OBP of liability for any earlier period. Automatum will step out on 30 days' notice
once the trigger is remedied, on an agreed plan.

**10.9 Insolvency.** The rights and licences under clauses 10.7 and 10.8 are licences of
intellectual property for the purposes of section 365(n) of the United States Bankruptcy Code,
and Automatum may elect to retain its rights under that section. Neither Party will assert
that those rights terminate automatically on the other's insolvency.

---

## 11. Fees, Invoicing and Payment

**11.1 The split.** For each Engagement Year the Collected Client Fees divide into Platform
Revenue and Service Revenue. Platform Revenue is $9,999 and belongs to Automatum whatever the
Client Status. Service Revenue divides by Client Status:

  a. **Net New Client** - the whole of the Service Revenue is retained by OBP, and Automatum
     takes no share of it.
  b. **Existing Automatum Client** - the Client already pays the Platform Revenue under its
     existing subscription, which continues unchanged, and the Service Revenue is divided 75%
     to OBP and 25% to Automatum in consideration of the Client relationship Automatum brings
     to the Engagement.

  Client Fees are the same at a given Tier whatever the Client Status. Schedule 2 states the
  figures and the rounding rule.

**11.2 Accrual.** Platform Revenue accrues pro rata across the 12 Engagement Months of an
Engagement Year at $833.25 per Engagement Month, and is not recovered from the earliest Client
payments in priority to OBP Service Revenue. Each Party's share of Service Revenue accrues
monthly at one twelfth of its annual share.

**11.3 Invoicing.** OBP will invoice Automatum monthly in arrears for the OBP Service Revenue
accrued in the preceding Engagement Month, with any amount payable under Schedule 2 for a
change in Tier.

**11.4 Payment.** Automatum will pay each undisputed invoice within 10 Business Days of
receiving the corresponding Client payment, and in any event within 60 days of the invoice
date whether or not the Client has paid. The 60 day longstop does not apply where the Client
has entered an insolvency process, or is withholding payment because of a dispute caused by
OBP's performance, in which case only the disputed portion may be withheld and only until
resolved.

**11.5 Automatum's collection obligations.** Automatum will invoice each Registered Client
within 5 Business Days of the due date under the Client Agreement, pursue collection
diligently, and give OBP a monthly statement showing for each Registered Client the Client
Status, Client Fees invoiced, amounts collected, Platform Revenue accrued and each Party's
share of Service Revenue accrued.

**11.6 No unilateral concessions.** Automatum will not settle, waive, discount or credit
Client Fees in a way that reduces OBP Service Revenue without OBP's prior written consent. If
it does, OBP Service Revenue is calculated as though the full Client Fees had been collected.

**11.7 Suspension.** OBP may not suspend any client-facing element of the Services for
non-payment unless payment is more than 60 days overdue and OBP has given 15 Business Days'
written notice to Automatum. OBP's remedy for sustained non-payment is termination of the
affected Engagement Order under clause 23.4 with transition assistance paid by Automatum.
Clause 7.4 applies throughout.

**11.8 Interest and set-off.** Interest accrues on overdue undisputed amounts at 1.5% per
month or the maximum rate permitted by law, whichever is lower. Neither Party may set off
except for sums that are liquidated, undisputed and due.

**11.9 Marketplace transaction of engagement fees.** Engagement fees are invoiced by Automatum
directly to each Client and are not transacted through AWS Marketplace, so no AWS Marketplace
listing fee applies to them, and the figures in Schedule 2 depend on that. If the Parties
later agree to transact engagement fees through AWS Marketplace, the allocation of the
resulting listing fee must be agreed under clause 14.5 before that begins.

**11.10 Taxes.** Each Party bears its own taxes on its own income. Amounts payable are
exclusive of Taxes, which Automatum will pay in addition where properly chargeable. If
withholding is required by law, the payer will gross up so the payee receives what it would
have received absent the withholding.

**11.11 Audit of billing.** Once per calendar year OBP may audit Automatum's records of Client
Fees invoiced and collected for Registered Clients, on 30 days' notice, in business hours, at
OBP's cost. If the audit shows an understatement of OBP Service Revenue above 5% for the
period audited, Automatum bears the audit cost and pays the shortfall with interest under
clause 11.8.

---

## 12. Reporting, Records and Audit

**12.1 Monthly report.** OBP will deliver the monthly reporting pack under clause 5.1(k). The
headline metric is the number of Offer-Issuing Partners, or the fallback metric where clause
14.2 applies, determined under Schedule 4.

**12.2 Source of truth.** The PSR is the source of truth for the metrics in Schedule 4. OBP
will reconcile it against AWS Marketplace seller reporting monthly and report any discrepancy.

**12.3 Disputes.** Automatum may dispute a reported figure within 15 Business Days of
receiving the report, resolved by reference to the event log under clause 10.5.

**12.4 Records and audit.** Each Party will keep accurate records of its performance for the
Term plus 3 years, and clause 10.5 governs the event log. Automatum may audit OBP's compliance
once per calendar year on 30 days' notice, in business hours, at Automatum's cost, and more
frequently after a Chronic Failure or security incident. Audits will not require disclosure of
another OBP client's confidential information.

---

## 13. AWS Program Compliance

**13.1 Compliance.** Each Party will comply with the AWS Program Terms applicable to it and
maintain the AWS registrations, certifications and programme statuses its role requires.

**13.2 No representation as AWS.** Neither Party will represent itself as AWS, as acting with
AWS's authority, or as able to bind AWS.

**13.3 AWS is not a party.** AWS is not a party to this Agreement and owes no obligation under
it. Nothing here or in the feasibility study for the Solution is a commitment by AWS,
including its reference to ACE opportunity submissions, which Schedule 8 records as a
programme assumption only.

**13.4 Solution identifier and overlap.** Opportunities for the Solution map to the single
solution identifier in Schedule 8. Where an opportunity falls within both the Solution and
another offering of either Party, the Parties will attribute it to one solution identifier,
applying the higher-tier offering's commitment structure, so no opportunity is billed twice.
Schedule 8 records the worked basis and the tie-break where tiers are equal.

**13.5 ACE integrity.** Neither Party will submit an ACE opportunity it knows or ought
reasonably to know to be inaccurate, duplicated or misattributed. Clause 5.1(j) governs the
account under which OBP files.

**13.6 AWS programme change.** If AWS changes an AWS Program Term, a Marketplace mechanism or
a programme rule so that a commitment in Schedule 1 or 4 becomes impossible or materially more
onerous, clause 14.3 applies.

---

## 14. Dependencies, Assumptions and Change Control

**14.1 Assumptions.** This Agreement is entered into on the assumptions in Schedule 8. An
assumption proving incorrect is not a breach, and the consequence is as stated in Schedule 8
or this clause.

**14.2 The Marketplace authorization dependency.** It is not yet confirmed whether, and by
what mechanism, a third-party reseller can issue offers against Automatum's listing while
Automatum is seller of record, and the Offer-Issuing Partner metric depends on it. Automatum
owns obtaining AWS's confirmation and will use reasonable efforts to obtain a written answer
by the longstop date in Schedule 8. If the mechanism is confirmed available by then, the
primary metric in Schedule 4 applies. If it is not, the fallback metric in Schedule 4 applies
automatically from the next Engagement Month, without further agreement, and neither Party is
in breach or liable to the other. Neither Party warrants the outcome.

**14.3 AWS programme change relief.** Where clause 13.6 applies, the affected obligation is
suspended, the Parties will meet within 10 Business Days to agree a change under clause 14.5,
and if they have not agreed within 30 days either may terminate the affected Engagement Order
on 30 days' notice, with Client Fees pro-rated to termination and no termination liability on
either side.

**14.4 Capacity test.** The service levels in Schedule 4 and the Tier 3 roster in Schedule 1
have not been capacity tested. Neither Party will publish the Tier table to the market, and no
Engagement Order may be executed at a Tier, until the capacity test in Schedule 8 has been
passed for that Tier.

**14.5 Change control.** Any change to the Services, the service levels, a Tier, the Schedules
or the commercial terms takes effect only in a change order signed by both Parties stating the
change and its effect on the Services and on OBP Service Revenue. Neither Party must agree a
change. Work outside the Services is chargeable at the Schedule 2 rate card.

---

## 15. Non-Circumvention and Non-Solicitation

**15.1 Purpose.** The appointment being non-exclusive, this clause is the protection each
Party receives in place of exclusivity. It is limited to Registered Clients and Recruited
Partners and does not restrain either Party's business generally.

**15.2 Registered Clients.** During the Term and for 24 months after the later of expiry and
termination of the relevant Engagement Order, OBP will not directly or indirectly solicit,
accept or provide channel services of the kind comprised in the Services to a Registered
Client or its affiliates, assist a third party to do so, or introduce a Registered Client to
another AWS Marketplace seller of record or prime provider for the purpose of procuring such
services. This does not apply to a Prior Relationship under clause 4.4, to general marketing
not targeted at Registered Clients, or to an approach the Registered Client initiates where
OBP declines it and notifies Automatum within 5 Business Days.

**15.3 Recruited Partners.** The population of AWS resellers is finite and a reseller
typically carries many vendor lines, so a general restriction on OBP working with firms it has
recruited would protect no legitimate interest. OBP may work with any Recruited Partner on
other engagements. During the Term and for 12 months after, OBP will not use Client Data to
solicit any partner away from a Registered Client's channel, procure or encourage a Recruited
Partner to terminate or breach its agreement relating to a Registered Client's channel, or
introduce a Recruited Partner to a competing software provider in the same solution category
as a Registered Client for the purpose of displacing that Client's product.

**15.4 Automatum's covenant.** During the Term and for 12 months after the later of expiry and
termination of the relevant Engagement Order, Automatum will not provide services of the kind
comprised in the Services to a Registered Client using OBP Materials outside the licence in
clause 16.3, or using personnel recruited from OBP in breach of clause 15.5.

**15.5 Personnel.** During the Term and for 12 months after the later of expiry and
termination of the relevant Engagement Order, neither Party will solicit for employment or
engagement any employee or contractor of the other who was materially involved in the
Services. This does not restrict general advertising not targeted at the other's personnel,
responses to unsolicited applications, or engaging a person who ceased to be engaged by the
other more than 6 months earlier. A Party in breach will pay the other 30% of the person's
first year total compensation.

**15.6 Liquidated damages.** If OBP breaches clause 15.2 in respect of a Registered Client,
OBP will pay Automatum $35,000 for that Client. Automatum's loss in that event is the loss of
its Platform Revenue stream and the cost of re-sourcing delivery; the Parties agree this
amount is a reasonable estimate of that loss made at the time of contracting, that the actual
loss would be difficult to determine, and that the amount is not a penalty and is not measured
against gross Client Fees, which are not Automatum's loss.

**15.7 Injunctive relief and reasonableness.** Damages alone may not be an adequate remedy for
breach of this clause or clause 17, and either Party may seek injunctive relief without
proving actual damage or posting a bond. The Parties have negotiated this clause as
sophisticated commercial parties with the benefit of advice; if a court finds any restriction
unreasonable in duration, scope or geography, it applies with the minimum modification needed
to make it enforceable.

---

## 16. Intellectual Property

**16.1 Background IP.** Each Party retains all rights in intellectual property it owned before
the Effective Date or develops independently of this Agreement. OBP retains all rights in the
OBP Materials, including the PSR, and Automatum retains all rights in the Automatum Brand.
Nothing here transfers ownership of either.

**16.2 Client Data.** Clause 10.3 governs.

**16.3 Client-facing deliverables.** OBP grants Automatum a perpetual, irrevocable,
non-exclusive, worldwide, royalty-free, sublicensable licence to use, copy, modify and
distribute the client-facing deliverables produced for a Registered Client, including that
Client's partner agreements, margin schedule, enablement content, playbooks and reports, for
that Client's own channel and for Automatum's performance of the relevant Client Agreement.
This licence survives termination.

**16.4 Feedback.** Each Party may use without restriction or obligation any suggestion or
feedback the other gives about its products or services.

---

## 17. Confidentiality

**17.1 Definition.** "**Confidential Information**" means non-public information disclosed by or
for a Party that is identified as confidential or that a reasonable person would understand to
be confidential. The Client Registry, the Recruited Partner roster, the Client Data, the terms
of this Agreement and each Engagement Order, and the existence and nature of OBP's role, are
each Confidential Information.

**17.2 Obligations.** The receiving Party will keep it confidential, use it only to perform or
enforce this Agreement, protect it with at least reasonable care, and disclose it only to
personnel, professional advisers and approved subcontractors who need it and are bound by
equivalent obligations.

**17.3 Exceptions.** These obligations do not apply to information that is or becomes public
without breach, was lawfully known without obligation before disclosure, is lawfully received
from a third party without obligation, or is independently developed without use of it. A
Party may disclose to the extent required by law, regulation or court order, giving reasonable
advance notice where lawfully permitted and disclosing only what is required.

**17.4 Duration.** These obligations continue for 5 years after termination, and indefinitely
for Client Data, Personal Data and trade secrets for as long as they remain trade secrets.

---

## 18. Data Protection and Security

**18.1 Roles.** For Personal Data in the Client Data, the Registered Client is the controller,
Automatum acts on the Client's instructions, and OBP acts as processor on Automatum's
documented instructions. Schedule 5 Part C states the processing particulars.

**18.2 OBP's obligations.** OBP will process Personal Data only on Automatum's documented
instructions; bind authorized persons to confidentiality; implement the security measures in
Schedule 5 Part C and not materially reduce them during the Term; assist Automatum with data
subject requests, security, breach notification and impact assessments; and on termination
return or delete Personal Data at Automatum's election, certifying deletion.

**18.3 Subprocessors.** OBP may engage subprocessors only from the list in Schedule 5 Part C
or with Automatum's prior written consent, will impose equivalent obligations on them, and
remains liable for them. OBP will give 30 days' notice of any proposed addition or
replacement, and Automatum may object on reasonable grounds.

**18.4 Security incidents.** OBP will notify Automatum without undue delay and within 24 hours
of becoming aware of a security incident affecting Client Data or Personal Data, with the
information Automatum needs to meet its own notification obligations.

**18.5 Compliance.** Each Party will comply with Privacy Law applicable to it, and the Parties
will promptly execute any further data processing agreement or transfer mechanism a Privacy
Law requires.

---

## 19. Warranties

**19.1 Mutual.** Each Party warrants that it is duly organized and validly existing, has full
power and authority to enter into and perform this Agreement, and that this Agreement is
binding and enforceable against it.

**19.2 OBP.** OBP warrants that it will perform the Services with reasonable skill and care
and to good industry practice; that its personnel are suitably skilled, qualified and
experienced; that clause 10.1 is accurate; and that the Services and OBP Materials infringe no
third party intellectual property rights.

**19.3 Automatum.** Automatum warrants that it holds and will maintain the listing, seller of
record registration, AWS Partner Network status and entitlements in clause 6.1; that it may
grant the licence in clause 7.1; and that each Client Agreement for a Registered Client
complies with clause 6.5.

**19.4 Compliance covenant.** Each Party will comply with applicable anti-bribery and
anti-corruption laws including the United States Foreign Corrupt Practices Act, applicable
sanctions and export control laws, and applicable modern slavery laws. OBP will apply the
partner due diligence in clause 5.1(a), will not onboard a partner owned or controlled by a
government official without Automatum's prior written consent, and will maintain records of
commission payments available to audit under clause 12.4.

**19.5 Disclaimer.** EXCEPT AS EXPRESSLY SET OUT IN THIS AGREEMENT, EACH PARTY DISCLAIMS ALL
WARRANTIES, EXPRESS, IMPLIED OR STATUTORY, INCLUDING MERCHANTABILITY, FITNESS FOR A PARTICULAR
PURPOSE AND NON-INFRINGEMENT. NEITHER PARTY WARRANTS ANY OUTCOME, INCLUDING ANY NUMBER OF
PARTNERS RECRUITED, AUTHORIZED, ISSUING OFFERS OR TRANSACTING, OR ANY LEVEL OF REVENUE.

---

## 20. Indemnities

**20.1 OBP.** OBP will defend Automatum against any third party claim, and indemnify it
against Losses finally awarded or agreed in settlement, arising from infringement of
intellectual property rights by the Services or OBP Materials; OBP's breach of clause 18; its
breach of an AWS Program Term; its breach of clause 15; and any claim by OBP's personnel that
they are or were employed by Automatum.

**20.2 Automatum.** Automatum will defend OBP against any third party claim, and indemnify it
against Losses finally awarded or agreed in settlement, arising from Automatum's breach of
clause 6.5 or 6.6, including any commitment made to a Client beyond Schedules 1 and 4; its
loss of, or a defect in, the listing, seller of record position or AWS Partner Network
entitlements; infringement of intellectual property rights by the Automatum Brand; and its
breach of an AWS Program Term.

**20.3 Conduct.** The indemnified Party will notify promptly, give sole control of defence and
settlement, and cooperate reasonably at the indemnifying Party's cost. The indemnifying Party
will not settle in a way imposing a non-indemnified obligation or admission of liability
without consent. Late notice reduces the indemnity only to the extent of resulting prejudice.

---

## 21. Limitation of Liability

**21.1 Uncapped.** Nothing limits liability for death or personal injury caused by negligence,
for fraud or fraudulent misrepresentation, or for any liability that cannot lawfully be
limited.

**21.2 Excluded losses.** Subject to clauses 21.1 and 21.4, neither Party is liable for
indirect, incidental, special, punitive or consequential loss, or for loss of profit, revenue,
anticipated savings, goodwill or business opportunity.

**21.3 Cap.** Subject to clauses 21.1 and 21.4, each Party's total aggregate liability is
limited, per Engagement Order, to the greater of 100% of the OBP Service Revenue paid and
payable for that Engagement Order in the 12 months before the event and $50,000.

**21.4 Super-cap.** Liability under clause 20, and for breach of clause 15, 17 or 18, is
limited to the greater of 200% of the OBP Service Revenue paid and payable in the 12 months
before the event and $250,000.

**21.5 Direct loss.** Notwithstanding clause 21.2, the following are recoverable as direct
loss: Service Credits flowing down under clause 8.4 within the caps in clause 8.5; the cost of
procuring replacement services after OBP's default, up to the cap in clause 21.3; and amounts
payable under clauses 15.5 and 15.6.

**21.6 Fees.** Amounts properly payable for the Services are not subject to these caps.

---

## 22. Insurance

OBP will maintain, with insurers of good standing, commercial general liability, professional
indemnity or errors and omissions, cyber liability, and employer's liability insurance, each
at the limit in Schedule 8, for the Term and 2 years after, will provide certificates on
request, and will notify Automatum promptly if any required cover lapses or is materially
reduced.

---

## 23. Term and Termination

**23.1 Term.** This Agreement begins on the Effective Date and continues for 24 months (the
"Initial Term"), then renews for successive 12 month periods unless either Party gives 90
days' notice before the end of the then current period. "Term" means the Initial Term with any
renewal.

**23.2 No orphaning.** Expiry or termination does not terminate an Engagement Order then in
effect. Each continues to its natural expiry and this Agreement continues to govern it, unless
that Engagement Order is itself terminated under this clause.

**23.3 Convenience.** Either Party may terminate this Agreement on 90 days' notice. Clause
23.2 applies, so this stops new Engagement Orders but does not end existing ones.

**23.4 Cause.** Either Party may terminate this Agreement, or a single Engagement Order,
immediately on notice if the other commits a material breach that is incapable of remedy, or
that is capable of remedy and is not remedied within 30 days of notice. Automatum may
terminate a single Engagement Order under clause 8.7, and OBP under clause 11.7.

**23.5 Insolvency.** Either Party may terminate immediately on notice if the other enters an
insolvency process, has a receiver, administrator or trustee appointed over a material part of
its assets, makes a general assignment for the benefit of creditors, or ceases business.
Clause 10.9 applies to licences granted here.

**23.6 Survival.** Clauses 1, 2, 7.7, 10.3, 10.5, 10.7, 10.8, 10.9, 11 for amounts accrued,
12.4, 15, 16, 17, 18, 20, 21, 22, 23.6, 24, 26 and 27 survive termination, with any provision
that by its nature is intended to.

---

## 24. Transition and Run-Off

**24.1 Transition period.** On expiry or termination of this Agreement or an Engagement Order,
OBP will provide transition assistance for 90 days, extendable by Automatum to 180 days on
notice given before the initial period ends. It is at OBP's cost where termination is for
OBP's breach or insolvency, and at the Schedule 2 rate card in every other case.

**24.2 Deliverables.** Transition assistance comprises a verified full export of the Client
Data under clause 10.4 with a restore test; knowledge transfer; handover of in-flight deal
registrations, CPPOs and Selling Authorization requests; run-out and reconciliation of accrued
partner commissions; introductions to the Recruited Partner roster on Automatum-branded
correspondence; disconnection of any CRM synchronization with a certificate of deletion; and
continuation of the licence in clause 16.3.

**24.3 Conduct.** Throughout, OBP continues to perform the Services, the service levels
continue to apply, and clause 7 continues to apply so that no Client or Recruited Partner
learns of OBP's role or of the termination except as Automatum directs.

---

## 25. Force Majeure

**25.1** Neither Party is liable for failure or delay caused by an event beyond its reasonable
control, including act of God, war, terrorism, civil unrest, epidemic, strike affecting a
third party, failure of a utility or telecommunications provider, and an unplanned AWS
platform outage (a "Force Majeure Event").

**25.2** A change by AWS to an AWS Program Term, a Marketplace mechanism or a programme rule
is **not** a Force Majeure Event; clauses 13.6 and 14.3 govern it.

**25.3** The affected Party will notify promptly and use reasonable efforts to mitigate. If a
Force Majeure Event continues 60 consecutive days, either Party may terminate the affected
Engagement Order on 30 days' notice.

---

## 26. General

**26.1 Governance and escalation.** The Parties will hold a monthly service review and a
quarterly business review, attended by the relationship managers in Schedule 7. A dispute is
escalated first to those managers, then to an executive of each Party, each stage allowing 10
Business Days. Neither Party may commence proceedings, other than for injunctive relief under
clause 15.7, until escalation is exhausted.

**26.2 Independent contractors.** The Parties are independent contractors. This Agreement
creates no partnership, joint venture, franchise, agency or employment relationship, except
the limited agency in clause 5.3 and any agency Automatum expressly grants in writing.

**26.3 Assignment.** Neither Party may assign or transfer this Agreement without the other's
prior written consent, not to be unreasonably withheld, except to an affiliate or a successor
in a merger or sale of substantially all its assets, on notice. If a Party is acquired by a
competitor of the other, the other may terminate on 60 days' notice, and clause 23.2 applies.

**26.4 Notices.** Notices must be in writing to the addresses in Schedule 8, by hand, by
recognized courier, or by email to the address stated for legal notices with a copy by
courier, effective on delivery or the next Business Day if delivered outside business hours.

**26.5 Entire agreement.** This Agreement is the entire agreement on its subject matter and
supersedes all prior discussions, understandings and representations, including the
feasibility study for the Solution, which is a discussion document and binds neither Party.
Neither Party has relied on any statement not set out here. This does not limit liability for
fraud.

**26.6 Amendment, waiver and severability.** An amendment is effective only in writing signed
by both Parties or by change order under clause 14.5. A waiver is effective only in writing,
and delay in exercising a right is not a waiver. If a provision is held invalid it is modified
to the minimum extent needed to make it enforceable, or severed if it cannot be, and the rest
continues; clause 15.7 applies to clause 15.

**26.7 No third party rights.** No person other than the Parties may enforce this Agreement,
except that a Registered Client may enforce clause 16.3 to the extent necessary to retain the
materials its channel runs on.

**26.8 Counterparts.** This Agreement may be executed in counterparts, including by electronic
signature, each an original and all together one instrument.

---

## 27. Governing Law and Jurisdiction

**27.1** This Agreement and any dispute arising out of or in connection with it, whether
contractual or non-contractual, is governed by the laws of the State of New York, without
regard to its conflict of laws rules. The United Nations Convention on Contracts for the
International Sale of Goods does not apply.

**27.2** Subject to clause 26.1, the state and federal courts in New York County, New York
have exclusive jurisdiction, and each Party irrevocably submits to it and waives any objection
based on venue or forum non conveniens.

**27.3** EACH PARTY IRREVOCABLY WAIVES ANY RIGHT TO TRIAL BY JURY IN ANY PROCEEDING ARISING
OUT OF OR RELATING TO THIS AGREEMENT.

---

# Schedule 1 - Services Description and Tier Matrix

## 1. The Solution

Managed Partner Development is a 12 month engagement under which Automatum builds and runs a
reseller channel on AWS for a Registered Client. Delivery is executed partner agreements and a
margin schedule, a live partner portal carrying onboarding, deal registration and commission
tracking, recruitment and enablement on a named cadence, a deal desk answering registrations,
and a monthly report whose headline number is the Offer-Issuing Partner count. Clause 5.1
states what OBP performs; this Schedule states what varies by Tier.

## 2. Tier matrix

| | Tier 1 | Tier 2 | Tier 3 |
|---|---|---|---|
| Client Fees | $2,500 per month | $5,000 per month | $7,500 per month |
| Net new Recruited Partners (ceiling) | up to 6 | up to 12 | uncapped roster |
| Training and enablement sessions | twice yearly | quarterly | monthly |
| Deal desk turnaround (total) | by close of the next Business Day | 8 Business Hours | 4 Business Hours |
| Channel Reviews | quarterly | monthly | fortnightly |

Everything else in clause 5.1 applies at every Tier. Clause 5.2 governs the partner numbers,
which are a ceiling and not a minimum.

## 3. Tier 3: uncapped roster, not unlimited rate

There is no contractual ceiling on the total number of Recruited Partners over a Tier 3
Engagement. OBP's obligation is throughput:

- OBP will process up to the Onboarding Throughput Rate in the Engagement Order per month and
  maintain up to the stated number of concurrent onboardings, as a first-in-first-out queue
  with queue position visible in the PSR. Requests above the rate are queued, and queuing is
  not a service level failure.
- Each Tier 3 Engagement Order states Baseline Assumptions for active partner count and
  monthly onboarding volume. If actual volume exceeds 150% of Baseline over any rolling 3
  months, either Party may invoke clause 14.5 and Automatum will use reasonable efforts to
  obtain an uplift or a move to a bespoke tier. If the Client will not agree, OBP's obligation
  is capped at Baseline plus fair use and Automatum carries the difference against the Client.
- Above the partner record threshold in the Engagement Order, incremental PSR licence cost
  passes through at cost under clause 14.5, unless OBP has warranted in the Engagement Order
  that it holds an unlimited-partner licence.

## 4. Partner milestones are targets, not warranties

The programme target is that a Recruited Partner achieves its first Transaction by the end of
the first quarter following its partner agreement signature date. This is a target supported
by a best efforts obligation and a reporting duty, not a warranty, because partner timelines
depend on the partner completing AWS registration, tax, banking and service-linked role
requirements, which is outside the control of OBP, Automatum and the Registered Client. Clause
8.3 stops the clock while a partner is pending those requirements.

## 5. Active Partner measurement

An Engagement Year is 12 months and an Active Partner must have transacted within 365 days, so
a partner recruited late in an Engagement cannot reach Active Partner status within it.
Accordingly Active Partner counts are measured on a trailing basis and again at a post-term
measurement date 3 months after the end of the Engagement Year; Recruited Partners onboarded
in the final 90 days of an Engagement Year are excluded from any in-term ratio expressed by
reference to Active Partners; and no fee, credit or commitment is calculated by reference to
an Active Partner count for a period in which the partner could not arithmetically have
qualified.

## 6. Out of scope

Anything not described in clause 5.1 or this Schedule, including bespoke integration work,
partner marketing spend, channel incentive funding, legal advice, and services for a Client's
direct customers, is chargeable under clause 14.5 at the Schedule 2 rate card.

---

# Schedule 2 - Commercial Terms

## 1. The split

Platform Revenue is $9,999 per Engagement Year at every Tier and belongs to Automatum in every
case, accruing at $833.25 per Engagement Month. Service Revenue is the balance and divides by
Client Status under clause 11.1.

**Net New Client - Service Revenue retained in full by OBP**

| Tier | Client Fees per month | per Engagement Year | Platform Revenue | Service Revenue to OBP | OBP per month |
|---|---|---|---|---|---|
| 1 | $2,500 | $30,000 | $9,999 | $20,001 | $1,666.75 |
| 2 | $5,000 | $60,000 | $9,999 | $50,001 | $4,166.75 |
| 3 | $7,500 | $90,000 | $9,999 | $80,001 | $6,666.75 |

**Existing Automatum Client - Service Revenue split 75% OBP / 25% Automatum**

The Client's existing subscription continues and is not part of the Engagement.

| Tier | Service Revenue | OBP (75%) | Automatum (25%) | OBP per month | Automatum total per year |
|---|---|---|---|---|---|
| 1 | $20,001 | $15,000.75 | $5,000.25 | $1,250.06 | $14,999.25 |
| 2 | $50,001 | $37,500.75 | $12,500.25 | $3,125.06 | $22,499.25 |
| 3 | $80,001 | $60,000.75 | $20,000.25 | $5,000.06 | $29,999.25 |

"Automatum total per year" is the $9,999 the Client already pays plus Automatum's 25% share,
stated so the two Client Statuses can be compared.

**Rounding.** Monthly accrual is one twelfth of the annual share rounded to the nearest cent,
and the final Engagement Month of each Engagement Year takes the rounding difference so the
twelve amounts total the annual share exactly.

These figures assume Client Fees are collected in full. Where they are not, Platform Revenue
and each Party's share of Service Revenue accrue only against Collected Client Fees, and the
shortfall is borne in the proportions above for the affected Engagement Month.

## 2. Worked example - Tier 2, full Engagement Year

**Net New Client.** $60,000 collected. Platform Revenue $9,999 to Automatum at $833.25 per
month. Service Revenue $50,001 retained by OBP at $4,166.75 per month.

**Existing Automatum Client.** The same $60,000 collected, the Client's existing subscription
continuing alongside. Service Revenue $50,001 divides $37,500.75 to OBP and $12,500.25 to
Automatum, and OBP invoices $3,125.06 monthly.

## 3. Worked example - Tier 1, terminated at the end of Engagement Month 3

| | Net New Client | Existing Automatum Client |
|---|---|---|
| Client Fees collected (3 x $2,500) | $7,500.00 | $7,500.00 |
| Platform Revenue accrued (3 x $833.25) | $2,499.75 | $2,499.75 |
| OBP Service Revenue accrued | $5,000.25 | $3,750.19 |
| Automatum share of Service Revenue | nil | $1,250.06 |

Neither Party recovers a further amount on account of the terminated Engagement except under
paragraph 4. Automatum has recovered $2,499.75 of the $9,999 it commits annually for its
Enterprise tier; that shortfall follows from the pro rata accrual in clause 11.2 and is
addressed, if at all, in the Client Agreement. Schedule 8 paragraph 3.2 records it as an open
item.

## 4. Early termination of an Engagement

| Cause | Outcome |
|---|---|
| Client terminates, or the Engagement expires | Each Party keeps what accrued. Any early termination charge collected is split in the proportions in paragraph 1 for that Tier and Client Status. |
| OBP's breach or Chronic Failure | Each Party keeps what accrued. OBP forfeits its share of any early termination charge and funds transition assistance under clause 24.1. |
| Automatum's breach or convenience | OBP is paid the greater of 3 months' OBP Service Revenue and the OBP Service Revenue for the remainder of the Engagement Year, plus evidenced demobilization costs. |
| No fault under clause 14.3 or 25.3 | Losses lie where they fall. Each Party keeps what accrued, and any early termination charge collected is split in the proportions in paragraph 1. |

## 5. Tier changes

**5.1 Upgrade.** Effective the first day of the Engagement Month following the Client's written
request. The enhanced service levels apply from 30 days after, so OBP can staff to them.

**5.2 Downgrade.** Effective only at an Engagement quarter boundary on 60 days' notice.

**5.3 Roster carry-over on downgrade.** The new Tier's partner ceiling applies to net new
Recruited Partners only. Continued management of an existing roster above the level the new
Tier supports is, by change order under clause 14.5, chargeable at the rate card; or resolved
by trimming the roster to the supported level within 60 days; or resolved by re-scoping as a
bespoke tier. Absent agreement within 60 days, OBP's obligation is limited to the new Tier's
supported level and Automatum carries the difference against the Client.

## 6. Rate card

Out of scope work under clause 14.5, chargeable transition assistance under clause 24.1, and
roster carry-over under paragraph 5.3 are charged at:

| Role | Day rate |
|---|---|
| Channel Manager | [to supply: day rate] |
| Deal Desk Analyst | [to supply: day rate] |
| Programme Director | [to supply: day rate] |
| PSR Administrator | [to supply: day rate] |

A day is 8 hours. Travel and expenses are chargeable at cost with prior written approval.

---

# Schedule 3 - White Label and Brand Rules

## 1. Identity

**1.1** OBP personnel doing client-facing or partner-facing work operate under the Automatum
Brand, using Automatum-supplied email addresses on an Automatum domain, Automatum calendar
invitations, document templates and email signatures. Automatum supplies and administers these
accounts and may revoke them at any time.

**1.2** OBP personnel are presented as "the Automatum channel team". They may say they are
part of that team, that they work on Automatum's channel programme, and that they are
contractors or partners of Automatum where directly asked. They may **not** state or imply
that they are employed by Automatum, sign any document as an employee or officer of Automatum,
or complete an employment verification, reference or immigration form in Automatum's name.

**1.3** Automatum does not set OBP personnel's hours, supervise their manner of work, conduct
their performance reviews or determine their compensation. OBP supplies their equipment and
working environment. Requests about an individual go to OBP's relationship manager under
clause 9.5, never to the individual.

## 2. Communications

**2.1** All Client and partner contact routes through a single Automatum-branded intake
channel stated in the Engagement Order, and OBP triages and routes under Schedule 4 paragraph
6.

**2.2** OBP will not initiate contact with a Registered Client outside that channel and the
named personas, and will not contact a Registered Client about any matter between the Parties.

**2.3** Calls and meetings are held on Automatum-branded conferencing, recorded per
Automatum's policy and applicable law.

**2.4** The CRM of record for client-facing activity is Automatum's, as stated in the
Engagement Order. OBP will not hold Registered Client contact records in its own systems
except within that Client's PSR tenant.

## 3. Materials

All client-facing and partner-facing materials carry the Automatum Brand and no OBP or PSR
vendor branding, subject to clause 10.2. Automatum supplies brand guidelines and approves
templates, and OBP will not vary an approved template in a way affecting the Automatum Brand
without approval. OBP will not publish, present or reuse any material produced for a
Registered Client outside that Engagement, subject to clause 16.1.

## 4. On termination

Clause 24.3 applies. OBP will return or destroy Automatum Brand assets, cease using the
Automatum domain and email accounts, and cooperate with introductions to the Recruited Partner
roster on Automatum-branded correspondence.

---

# Schedule 4 - Service Levels, Measurement and Credits

## 1. Deal desk turnaround

A Deal Desk Request is "Complete" when it contains every item in the request form in the
Engagement Order. The clock starts when a Complete request is received through the intake
channel, or at the start of the next Business Hour if received outside Business Hours, and
stops when the responsible Party delivers its output. Turnaround splits into two legs, because
Automatum issues offers as seller of record and OBP cannot control issuance.

| Tier | Total turnaround | OBP leg: Complete request to Issuance-Ready Package | Automatum leg: package to offer issued |
|---|---|---|---|
| 1 | By close of the next Business Day after receipt | 6 Business Hours | 3 Business Hours |
| 2 | 8 Business Hours | 5 Business Hours | 3 Business Hours |
| 3 | 4 Business Hours | 2.5 Business Hours | 1.5 Business Hours |

Tier 1 is satisfied if the offer is issued by close of the Business Day following the Business
Day of receipt, permitting up to approximately 16 Business Hours elapsed; Tier 2 is a fixed 8
Business Hours from receipt. The two are defined separately because a Business Day and 8
Business Hours would otherwise be the same period. Clause 8.2 governs attribution between legs.

## 2. Selling Authorizations

At every Tier: OBP leg, request to submission to Automatum, 2 Business Days; Automatum leg,
submission to issuance, 2 Business Days.

## 3. Cadence commitments

Training and enablement sessions and Channel Reviews run at the Tier cadences in the Schedule
1 tier matrix, and the monthly reporting pack is due within 5 Business Days of month end at
every Tier. A cadence commitment is met if the session or review is offered on at least two
proposed dates within the period and is held, or is not held because of the Registered
Client's unavailability.

## 4. Metric dictionary

**4.1 Primary metric - Offer-Issuing Partners.** The metric the feasibility study calls
"partners issuing offers": the number of distinct Recruited Partners that created at least one
private offer or CPPO against the Registered Client's product during the reporting month, read
from the PSR offer record and reconciled against AWS Marketplace seller reporting. It applies
only if the dependency in clause 14.2 is confirmed available by the longstop date in Schedule
8.

**4.2 Fallback metric - Partner-Attributed Transactions.** The number of Transactions issued
by Automatum against a Selling Authorization or private offer attributed to a named Recruited
Partner through AWS Partner Revenue Measurement and a matching opportunity identifier during
the reporting month, read from the PSR and reconciled against AWS Marketplace seller
reporting. It applies automatically under clause 14.2 if the dependency is not confirmed by
the longstop date.

**4.3 Supporting measures.** The monthly reporting pack also reports Recruited Partners
onboarded in the month and cumulatively; partners pending AWS prerequisites; Active Partners
measured under Schedule 1 paragraph 5; deal registrations received, approved and declined;
Deal Desk Requests and turnaround performance by leg; Selling Authorizations requested and
issued; and channel revenue attributed.

## 5. Support model

| Matter | First line | Second line |
|---|---|---|
| Channel, partner queries, enablement | OBP | Automatum |
| Deal desk and deal registration | OBP | Automatum |
| Partner System of Record | OBP | PSR vendor via OBP |
| Listing, offer issuance, Selling Authorizations | Automatum | AWS via Automatum |

All contact arrives through the single Automatum-branded intake under Schedule 3 paragraph
2.1. OBP triages within 2 Business Hours and routes to the correct first line. A misrouted
matter is handed over within 2 Business Hours of identification, and the receiving Party's
clock starts on handover.

| Priority | Definition | Response | Updates |
|---|---|---|---|
| P1 | A Registered Client cannot transact, or the PSR is unavailable | 1 Business Hour | every 4 Business Hours |
| P2 | A partner is blocked on a live deal | 4 Business Hours | daily |
| P3 | A partner or Client query not blocking a deal | 1 Business Day | every 3 Business Days |
| P4 | Request for information or change | 3 Business Days | weekly |

## 6. Credits

Clause 8.4 governs flow-down, clause 8.5 the caps and clause 8.7 Chronic Failure. A Service
Credit is measured against the Registered Client's gross monthly Client Fees, being the basis
on which the Client Agreement computes it. There is no earn-back.

---

# Schedule 5 - Partner System of Record

## Part A - Vendor and licence

| Item | Detail |
|---|---|
| PSR vendor legal name | [to supply: PSR vendor legal name] |
| Product name and edition | [to supply: product name and edition] |
| OBP's licence term and expiry | [to supply: licence term and expiry date] |
| Unlimited partner records? | [to confirm: yes, or the threshold above which cost passes through under Schedule 1 paragraph 3] |
| Seats included per Tier | [to confirm: seats for Tier 1 / Tier 2 / Tier 3] |
| White label branding and custom domain supported? | [to confirm: yes or no, and any element that cannot be unbranded under clause 10.2] |
| Tenancy model | [to confirm: per-Client tenant or shared with logical separation] |
| Hosting regions | [to supply: hosting regions] |
| Security certifications | [to supply: SOC 2 Type II / ISO 27001 status and report date] |

Clause 10.1 warrants the licence term outlasts the longest Engagement Order plus the
transition period. Licence cost sits within the OBP Service Revenue, subject to Schedule 1
paragraph 3.

## Part B - Functional scope

For each Registered Client, under the Automatum Brand: a partner portal; onboarding workflow
including AWS registration, tax, banking and service-linked role status tracking; deal
registration, approval and decline workflow with audit history; commission tracking and
accrual against the margin schedule; offer and Selling Authorization records, being the audit
source for the metrics in Schedule 4 paragraph 4; synchronization with HubSpot and Salesforce
as specified in the Engagement Order, which states whose instances, which is master, the
direction, and who holds the licences, and where a Registered Client's instance is in scope
the Client's cooperation is a dependency to which clause 6.7 applies; reporting supporting the
monthly pack; and the append-only event log required by clause 10.5.

## Part C - Data, processing and security

**1. Processing particulars.** Subject matter: operation of a reseller channel for a
Registered Client. Duration: the Engagement plus the transition period plus the retention
period in clause 10.5. Nature and purpose: partner onboarding, deal registration, commission
tracking, offer tracking and reporting. Personal Data: business contact details, job titles
and professional profile information of Recruited Partner and Registered Client personnel.
Data subjects: personnel of Recruited Partners, Registered Clients and the Parties. No special
category data.

**2. Export schema.** OBP maintains a documented, versioned machine-readable export schema
with a data dictionary covering partner records, partner agreements and status, deal
registrations, the commission ledger and accruals, opportunity records and ACE identifiers,
offer and Selling Authorization records, attribution configuration and outputs, and the event
log. Clause 10.4 governs frequency and restore testing.

**3. Security controls.** Role-based access control with least privilege and quarterly access
review; multi-factor authentication for administrative access; encryption in transit using
current TLS and at rest; centralized logging and alerting; documented vulnerability management
with remediation timescales by severity; annual penetration test with a summary available to
Automatum; background screening of personnel with access; documented incident response; and a
secure development lifecycle for any OBP-built extension.

**4. Subprocessors.** [to supply: subprocessor list, with name, role and processing location].
Clause 18.3 governs changes.

## Part D - Form of vendor continuity undertaking

The undertaking required by clause 10.7 is given by the PSR vendor in favour of Automatum and
provides, in substance, that on OBP's insolvency, cessation of business, or termination of
this Agreement for OBP's breach, the vendor will at Automatum's election and on its notice
contract directly with Automatum or its nominee for the remainder of the affected Engagement
Orders plus the transition period in clause 24.1; at the rates OBP then pays or rates
benchmarked to the vendor's then current list price for equivalent volume, whichever is lower;
preserve each affected Registered Client's tenant and data intact and available without
interruption; and not delete or withhold tenant data for at least 90 days after the triggering
event.

[to supply: executed vendor continuity undertaking, to be attached]

---

# Schedule 6 - Form of Engagement Order

**Engagement Order No. [ ] under the White Label Delivery Agreement dated [to supply:
Effective Date]**

| Item | Detail |
|---|---|
| Registered Client legal name | |
| Client Agreement reference and date | |
| Confirmation the Client Agreement complies with clause 6.5 | Yes / No |
| Tier | 1 / 2 / 3 |
| Client Status (clause 4.7) | Net New Client / Existing Automatum Client |
| Client Source (clause 4.7) | OBP-Sourced / Automatum-Sourced / Joint or ACE-Sourced |
| Engagement start date and Engagement Year dates | |
| Client Fees per month | |
| Platform Revenue per Engagement Year | $9,999 |
| Service Revenue split | 100% OBP (Net New) / 75% OBP, 25% Automatum (Existing) |
| OBP Service Revenue per month | |
| Named Channel Manager and Backup | |
| Named Deal Desk Owner (Tier 3) and coverage window | |
| PSR seats, tenancy and partner record threshold | |
| HubSpot and Salesforce instances, master and sync direction | |
| Intake channel and CRM of record | |
| Ideal partner profile | |
| Deal Desk Request and Issuance-Ready Package forms | Attached |
| Baseline Assumptions and Onboarding Throughput Rate (Tier 3) | |
| Unlimited partner licence warranted? (Tier 3) | Yes / No |
| Prior Relationships notified under clause 4.4 | |
| Variations under clause 2.2, if any | |

Signed for and on behalf of Automatum and OBP.

---

# Schedule 7 - Key Personnel, Coverage and Capacity

## 1. Named roles

| Role | Tiers | Committed hours per Engagement Month | Coverage window |
|---|---|---|---|
| Channel Manager | 1, 2, 3 | [to confirm: hours per Tier] | Business Hours |
| Backup Channel Manager | 1, 2, 3 | [to confirm: hours per Tier] | Business Hours |
| Deal Desk Owner | 3 | [to confirm: hours] | [to confirm: coverage window, stated in the Engagement Order] |

## 2. Capacity ratios

Engagements are weighted by Tier: Tier 1 counts 1, Tier 2 counts 2, Tier 3 counts 4. A Channel
Manager may hold at most 12 weighted points and at most 4 Tier 3 Engagements, whichever binds
first. Clause 9.7 makes exceeding a ratio an independent breach.

## 3. Relationship managers

| Party | Relationship manager | Executive for escalation under clause 26.1 |
|---|---|---|
| Automatum | [to supply: name and title] | [to supply: name and title] |
| OBP | [to supply: name and title] | [to supply: name and title] |

---

# Schedule 8 - Dependencies, Assumptions and Open Items

## 1. How to read this Schedule

**to supply** is a fact awaiting transcription. **to confirm** is a commercial figure needing
a decision or test; each one inside an operative obligation takes the default in paragraph 5
if this Agreement is executed first. **to verify** is an external dependency, resolved by
clause 14 and never by assumption. This register must read nil before execution.

## 2. Dependencies to verify

| Item | Owner | Longstop | Consequence if not verified |
|---|---|---|---|
| Whether, and by what mechanism, a third-party reseller can issue offers against Automatum's listing while Automatum is seller of record | Automatum | [to supply: longstop date, the earlier of 45 days from the Effective Date and first publication of the Tier table] | The fallback metric in Schedule 4 paragraph 4.2 applies automatically under clause 14.2. Neither Party is in breach or liable. |
| PSR vendor continuity undertaking under clause 10.7 | OBP | Before the first Engagement Order | Automatum may proceed, decline, or require an alternative PSR. |
| Capacity test of Schedule 4 and the Tier 3 roster | Joint | Before publication of the Tier table | No Tier table published and no Engagement Order executed at an untested Tier. Clause 14.4. |
| Insurance certificates under clause 22 | OBP | Before the first Engagement Order | No Engagement Order is executed. |

## 3. Assumptions

**3.1 Engagement fees are invoiced directly.** Schedule 2 assumes engagement fees are invoiced
by Automatum to the Client directly, not transacted through AWS Marketplace, so no listing fee
reduces them. The study's arithmetic assumes this: at Tier 1 the stated $9,999 and $20,001 sum
to exactly $30,000. If engagement fees are later transacted through Marketplace, a listing fee
of approximately 3% would be $900, $1,800 and $2,700 per Engagement Year at Tiers 1 to 3, and
its allocation must be agreed under clause 11.9.

**3.2 Automatum's Enterprise tier commitment.** Automatum's $9,999 is assumed to be committed
annually. Under the pro rata accrual in clause 11.2, an Engagement terminating at the end of
Engagement Month 3 recovers only $2,499.75 of it. **Open commercial item: whether the Client
Agreement carries a minimum term with an early termination charge, whether Automatum's
Enterprise tier is monthly cancellable, or whether Automatum accepts early churn as its own
loss.** Default if not closed: Automatum bears the shortfall.

**3.3 Tier 1 cost to serve.** PSR licensing was stress-tested against Tier 1; labour was not.
Whether Tier 1 is viable at $20,001 of Service Revenue per Engagement Year is an open
commercial item, and neither Party warrants that it is.

**3.4 Automatum's margin as prime.** On a Net New Client, Automatum's return is the Platform
Revenue alone, 11.1% of gross at Tier 3, against client credit risk, service level liability
and collection risk. Whether that is margin or a pass-through of Automatum's own cost is an
open commercial item; clause 11.1(b) settles the equivalent question for an Existing Automatum
Client.

**3.5 Stress testing at the wrong end.** PSR licensing was stress-tested against Tier 1, the
most predictable case, while Tier 3 carries an uncapped roster. If the PSR is priced per
partner or seat, Tier 3 is where the model breaks. Schedule 5 Part A records the question and
Schedule 1 paragraph 3 provides the pass-through.

**3.6 AWS is not committed.** The study's reference to five ACE opportunity submissions is a
programme assumption only, not a commitment by AWS, Automatum or OBP. Clause 13.3 governs.

**3.7 Partner AWS prerequisites.** Recruited Partner timelines depend on the partner
completing AWS registration, tax, banking and service-linked role requirements, outside the
control of either Party and the Registered Client. Clause 8.3 and Schedule 1 paragraph 4
govern.

## 4. Facts to supply

| Item | Owner |
|---|---|
| Both Parties' legal entity names, entity types, states of organization, registration numbers | Joint |
| Principal and notice addresses, including the address for legal notices under clause 26.4 | Joint |
| Signatory names and titles, and the Effective Date | Joint |
| Named Channel Manager, Backup and Tier 3 Deal Desk Owner per Engagement | OBP |
| Relationship managers and escalation executives (Schedule 7 paragraph 3) | Joint |
| PSR vendor details and certifications (Schedule 5 Part A) | OBP |
| Subprocessor list (Schedule 5 Part C paragraph 4) | OBP |
| Rate card day rates (Schedule 2 paragraph 6) | OBP |
| Insurance limits for each required cover | Joint |
| AWS Marketplace listing identifier and seller of record entity | Automatum |
| The APN account under which OBP files partner-sourced opportunities under clause 5.1(j) | Automatum |
| The solution identifier under clause 13.4, and the tie-break where overlapping offerings are of equal tier | Automatum |
| Longstop date for the dependency in paragraph 2 | Automatum |

## 5. Figures to confirm

| Item | Default if not confirmed at execution |
|---|---|
| PSR seats per Tier | Seats sufficient for the named personnel, the Client's nominated administrators and every Recruited Partner on the roster |
| Named-owner committed hours per Tier | Hours sufficient to meet Schedule 4 for the Tier, with clause 9.7 continuing to bind |
| Tier 3 Deal Desk Owner coverage window | Business Hours |
| Onboarding Throughput Rate and concurrent onboardings (Tier 3) | 4 onboardings per month and 6 concurrent |
| Tier 3 Baseline Assumptions | 30 active partners and 4 onboardings per month |
| PSR partner record threshold for cost pass-through | The threshold in OBP's licence, failing which no pass-through applies |
| Unlimited partner licence warranty (Tier 3) | Not warranted, so the pass-through in Schedule 1 paragraph 3 applies |

---

# Schedule 9 - Execution

The Parties have executed this Agreement as of the Effective Date.

<br>

**For and on behalf of [to supply: Automatum legal entity name]**

| | |
|---|---|
| Signature | |
| Name | [to supply: signatory name] |
| Title | [to supply: signatory title] |
| Date | |

<br>

**For and on behalf of [to supply: Only Best Practices legal entity name]**

| | |
|---|---|
| Signature | |
| Name | [to supply: signatory name] |
| Title | [to supply: signatory title] |
| Date | |
